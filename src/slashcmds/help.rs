...
pub async fn help(ctx: &Context, msg: &CommandInteraction) -> CommandResult {
    ...
    let hello_embed = CreateEmbed::new()
        .color(COLOR_OKAY)
        .description(
            format!("Hello! I can compile code for you. To compile code, \
            use the `{0}compile` command. type `{0}help compile` for more information.",
            botinfo.get("BOT_PREFIX").unwrap()),
        )
        .thumbnail(ICON_HELP);

    let markdown_embed = CreateEmbed::new()
        .color(COLOR_WARN).description(
    ...
    buttons.push(
        CreateButton::new_link(invite_link)
            .label("Invite me")
    );
    buttons.push(
        CreateButton::new_link(dbl_link)
            .label("Vote for us")
    );
    buttons.push(
        CreateButton::new_link(github_link)
            .label("GitHub")
    );
    buttons.push(
        CreateButton::new_link(stats_link)
            .label("Stats")
    );
    ...
    msg.create_response(&ctx.http, CreateInteractionResponse::Message(response)).await?;
