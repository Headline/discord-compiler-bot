...
pub async fn botinfo(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    ...
    let invite = env::var("INVITE_LINK")
        .expect("Expected invite link envvar");
    let topgg = env::var("DISCORDBOTS_LINK")
        .expect("Expected top.gg link envvar");
    let github = env::var("GITHUB_LINK")
        .expect("Expected github link envvar");
    let stats = env::var("STATS_LINK")
        .expect("Expected stats link envvar");
    ...
    let msg = msg.channel_id.send_message(&ctx.http, |m : &mut CreateMessage|  {
        m.embed(|e : &mut CreateEmbed| {
            ...
            let str = format!("Built from commit [{}]({}{}{})", short, github, "/commit/", hash);
            e.fields(vec![
